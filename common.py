...
def read_data(model_dir, fields, snap, subvolumes, name_file):
    """Read the second-try.properties*hdf5 file for the given run"""

    data = collections.OrderedDict()
    for idx, subv in enumerate(subvolumes):

        fname = os.path.join(model_dir, name_file+'_%03d.properties.%01d' % ( snap, subv))
        ...
        with h5py.File(fname, 'r') as f:
            for dsname in fields:
                ...
                if l is None:
                    ...
                    l = np.concatenate([l, f[full_name].value])
                data[full_name] = l
